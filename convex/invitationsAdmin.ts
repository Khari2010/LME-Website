...
export const resendInvitation = action({
  ...
  handler: async (ctx, { clerkInvitationId, invitedBy }): Promise<{ invitationId: string; messageId: string | undefined }> => {
    ...
    // Now create a fresh one — re-uses the same Clerk + Resend + Convex logic
    return createAndSendInvitation(ctx, {
